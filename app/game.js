...
class Game {
	...
	loadBackgrounds() {
		...
		this.backgrounds.win = new PIXI.Sprite.from(PIXI.loader.resources['assets/images/backgrounds/win.jpg'].texture);
	...
	gameLoop() {
		this.app.ticker.add(() => {
			...
			if (
				...
				this.opponent.actions.hit.currentFrame + 1 === this.opponent.actions.hit.totalFrames
			) {
			...
			if (
				...
				this.opponent.actions.highhit.currentFrame + 1 === this.opponent.actions.highhit.totalFrames
			) {
				this.opponent.actions.stance.visible = true;
				this.opponent.actions.highhit.visible = false;
			}

			if (this.action === 'jump' && this.keys.up.isDown && this.keys.right.isDown) {
			...
			if (this.action === 'jump' && this.keys.up.isDown && this.keys.left.isDown) {
				this.characters.forEach(character => {
					if (character.actions.jump) {
				...
				case 'walk-right':
					this.characters.forEach(character => {
						if (character.actions.walk) {
							...
							collision = this.coli.rectangleCollision(character, this.opponent);
				...
				case 'kick':
					this.characters.forEach(character => {
						if (character.actions.kick) {
							...
							if (character.actions.kick.currentFrame + 1 === character.actions.kick.totalFrames) {
								this.action = 'stance';
							}
				...
				case 'punch':
					this.characters.forEach(character => {
						if (character.actions.punch) {
							...
							if (character.actions.punch.currentFrame + 1 === character.actions.punch.totalFrames) {
								this.action = 'stance';
							}
				...
				case 'raise':
					this.characters.forEach(character => {
						if (character.actions.raise) {
							...
							if (character.actions.raise.currentFrame + 1 === character.actions.raise.totalFrames) {
								this.action = 'stance';
							}
						}
					});
	...
	youWin() {
		...
		let titleContinue = this.textObj.customText('Press Enter to Restart', 'center', 90);
